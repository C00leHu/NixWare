...
def get_remote_version():
    ...
    try:
        url = f"https://raw.githubusercontent.com/{GITHUB_USER}/{GITHUB_REPO}/{BRANCH}/version.txt"
    ...
    try:
        ...
        for item in data.get('tree', []):
            if item['type'] == 'blob':
                files.append({
                    'path': item['path'],
                    'url': f"https://raw.githubusercontent.com/{GITHUB_USER}/{GITHUB_REPO}/{BRANCH}/{item['path']}"
                })
